...
if forhistory && analysisdate==0 % added by Mark: only add to hist file if working on current date
    l=dlmread('EV_estimate_history.csv');
    if l(end,1) == polldata(1,5)
        l(end,:)=[] % delete the last row so that we can overwrite it
    end
    l(end+1,:)=[polldata(1,5) outputs]
    dlmwrite('EV_estimate_history.csv',l)
end

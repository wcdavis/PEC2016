...
from ev_update_polls import state_names

huffpo_url_pattern = "http://elections.huffingtonpost.com/pollster/2016-%s-president-trump-vs-clinton"
...
def display_state(jvdisplay, state):
    ...
    if state == "FL":
        data_url = "http://elections.huffingtonpost.com/pollster/2016-florida-presidential-general-election-trump-vs-clinton"
    
    print state, data_url
    if margin > 0:
        jvdisplay.write('<td><a href="%s" style="text-decoration: none; color: blue">'
                'Clinton +%s%%</a></td>' % (data_url, margin))
    elif margin < 0:
        jvdisplay.write('<td><a href="%s" style="text-decoration: none; color: red">'
                'Trump +%s%%</a></td>' % (data_url, -margin))
    else:
...
jvfile = open("EV_jerseyvotes.csv")
...
jvdisplay = open("EV_jerseyvotes.html", "w")
